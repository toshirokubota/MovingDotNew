...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	cv::Mat edge2;
	cv::Canny(im2, edge2, low, high); // , 7, true);
	...
	if (nlhs >= 3)
	{
		plhs[2] = StoreData(dx, mxSINGLE_CLASS, ndim, dims);
	}
	if (nlhs >= 4)
	{
		plhs[3] = StoreData(dy, mxSINGLE_CLASS, ndim, dims);
	}
	if (nlhs >= 5)
	{
		plhs[4] = StoreData(gr, mxSINGLE_CLASS, ndim, dims);
	}
	if (nlhs >= 6)
	{
		plhs[5] = StoreData(emag, mxSINGLE_CLASS, ndim, dims);
	}
	mexUnlock();
}
